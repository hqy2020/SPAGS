...
### 2026-04-28 23:00 | 🔬 foot_N1_adm64_tv0.0_5000it — 更长训练≠更好ADM

| **假设**: ADM峰值在2000it, 5000it训练能否维持或超越?

| **参数**: feat_dim=64, r_max=1.0, tv_weight=0.0, gaussiansN=1, foot_50_3views, iterations=5000

| **完整轨迹**:
|   - ITER 1000: PSNR2D=**29.94**, SSIM2D=0.9023 (峰值)
|   - ITER 2000: PSNR2D=29.91, SSIM2D=0.8903 (-0.03dB)
|   - ITER 3000: PSNR2D=29.77, SSIM2D=0.8867 (-0.17dB)
|   - ITER 4000: PSNR2D=29.76, SSIM2D=0.8843 (-0.18dB)
|   - ITER 5000: PSNR2D=29.72, SSIM2D=0.8826 (-0.22dB)

| **关键发现**:
|   - PSNR在ITER 1000后**单调递减**, 无二次峰值
|   - 从1000it到5000it共下降-0.22dB（-0.055dB/1000it速率）
|   - 更长训练(>3000it)对ADM是净负收益
|   - 对比之前30.05@3000it: 本次29.77@3000it差异~0.3dB, 归因于随机种子波动
|   - **ADM过拟合在~1000-2000it内, 3000it已充分, 5000it完全不需要**

| **对训练策略的影响**:
|   - 建议ADM训练在1500-2000it停止 (早停)
|   - 或使用更积极的学习率衰减
|   - 不需要做5000it以上的超长训练实验

| **状态**: ✅ keep (确认3000it为ADM最优训练长度)
